...
def test_deprecator_registry_creation() -> None:
    """Test that we can create a DeprecatorRegistry instance."""
    # This test will fail initially as DeprecatorRegistry doesn't exist yet
    from deprecator._registry import DeprecatorRegistry

    registry = DeprecatorRegistry()
    assert isinstance(registry, DeprecatorRegistry)


def test_deprecator_registry_for_package() -> None:
...
def test_custom_registry_creation() -> None:
    ...
    framework_registry = DeprecatorRegistry()
...
def test_registry_with_framework_name() -> None:
    ...
    django_registry = DeprecatorRegistry(framework_name="Django")
    flask_registry = DeprecatorRegistry(framework_name="Flask")
    unnamed_registry = DeprecatorRegistry()

    # Check framework names
    assert django_registry.framework_name == "Django"
    assert flask_registry.framework_name == "Flask"
    assert unnamed_registry.framework_name is None
...
def test_deprecation_tracking() -> None:
    ...
    assert dep_info1["warning"] is deprecation1
    assert dep_info2["warning"] is deprecation2

    # Should have importable_name field (initially None since not specified)
    assert "importable_name" in dep_info1
    assert "importable_name" in dep_info2
    assert dep_info1["importable_name"] is None
    assert dep_info2["importable_name"] is None


def test_deprecation_tracking_with_importable_names() -> None:
    ...
    deprecation1 = deprecator_instance.define(
        ...
        gone_in="2.0.0",
        warn_in="1.5.0",
        importable_name="my_package.module.foo",
    )

    deprecation2 = deprecator_instance.define(
        "Class Bar is deprecated",
        gone_in="3.0.0",
        warn_in="2.0.0",
        importable_name="my_package.classes.Bar",
    )

    # Should be able to retrieve deprecations with importable names from the deprecator
    tracked_deprecations = deprecator_instance.get_tracked_deprecations()

    foo_deprecation = next(
        d
        for d in tracked_deprecations
        if d["importable_name"] == "my_package.module.foo"
    )
    bar_deprecation = next(
        d
        for d in tracked_deprecations
        if d["importable_name"] == "my_package.classes.Bar"
    )

    assert foo_deprecation["warning"] is deprecation1
    assert bar_deprecation["warning"] is deprecation2
